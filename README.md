...
このソフトウェアは **無保証** で提供されます（AS IS）。

- このソフトの使用、または使用不能によって生じたいかなる損害（**データの消失・破損、ハードウェアの故障、業務の中断、医療上の判断ミス、その他間接損害を含む**）について、**開発者は一切の責任を負いません**。
- 本ソフトは個人・家族向けの記録ツールであり、**医療機器・診断機器ではありません**。記録された数値の医学的解釈・判断は専門家にご相談ください。
- 使用者は **自己責任** で導入・運用してください。重要なデータは必ず **定期バックアップ** を取ってください（[Data Persistence](#data-persistence) セクション参照）。
- バグ報告は Issues で歓迎しますが、**修正・対応の保証はありません**（個人プロジェクトのため）。

詳細な法的条項は [LICENSE](LICENSE)（MIT License）の `THE SOFTWARE IS PROVIDED "AS IS"...` 以降を参照してください。
