...
- 「何ができるか」は [spec.md](spec.md)、「守るべき制約」は `biolog_streamlit/CLAUDE.md`（開発リポジトリのみ・非公開）を参照してください。本書はその2つの**間を埋める、コードの動作説明**です。
...
`biolog_streamlit/CLAUDE.md` に10個の禁止事項があります。**どのコードがそれを担保しているか**を対応させます。
...
| 9 | キャッシュ無効化は per-function `.clear()` | `cache.py:clear_health_caches()` |
...
### 4.4 cache.py — 読み取りキャッシュ（18行）

```python
@st.cache_data(ttl=10)
def fetch_range_data(start, end): …
@st.cache_data(ttl=10)
...
10秒間のキャッシュ。登録・更新・削除の直後に `clear_health_caches()` を呼んで即座に反映させます。

**per-function の `.clear()` を使うことがルール9**です（`session_state` にバージョン番号を持たせる方式は禁止）。
...
| `biolog_streamlit/CLAUDE.md` | 設計上の絶対ルールとデバッグチェックリスト（開発リポジトリのみ・非公開） |
